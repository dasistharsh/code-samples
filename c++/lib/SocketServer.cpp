#include "SocketServer.h"
  ...
  */
  ...
  int SocketServer::socket_bind(int port)
  {
    ...
    {
      perror( "[#] bind() ");
      cerr << "[#] Perhaps it means that you were running server earlier and you didn't properly closed that port."<<endl;
      cerr << "[#] Now systemd must restart me... Bye" << endl;
      // Quick patch below 
      string kill_process = "sudo kill -9 `sudo netstat -laptn | grep -i listen | grep -i " + std::to_string(PORT) + " | rev | awk '{print $1}' | rev |  cut -d '/' -f1`";
